                                ...
                                <div key={sub.id} style={{background:T.bg3,borderRadius:10,padding:"12px 14px",border:"1px solid "+T.bd}}>
                                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,flexWrap:"wrap"}}>
                                    ...
                                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                                      ...
                                      {sub.file_url&&sub.file_url.toLowerCase().endsWith(".pdf")&&<button onClick={async()=>{if(addNotif)addNotif("🤖","AI Grading","Analysing submission, please wait...");try{const resp=await fetch(sub.file_url);const blob=await resp.blob();const b64=await new Promise(r=>{const reader=new FileReader();reader.onload=()=>r(reader.result.split(",")[1]);reader.readAsDataURL(blob);});const gemKey=import.meta.env.VITE_GEMINI_KEY;let txt="{}";
const gemRes=await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key="+gemKey,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{inline_data:{mime_type:"application/pdf",data:b64}},{text:"You are an academic examiner. Grade this student submission.\n\nAssignment: "+a.title+"\nInstructions: "+(a.description||"Complete the assignment as instructed.")+"\nMax Marks: "+a.max_marks+"\n\nReturn ONLY this JSON: {\"marks\": NUMBER, \"feedback\": \"STRING\"}"}]}],generationConfig:{maxOutputTokens:500}})});
if(gemRes.status===429||gemRes.status===404){const clKey=import.meta.env.VITE_ANTHROPIC_KEY;const clRes=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":clKey,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:500,messages:[{role:"user",content:[{type:"document",source:{type:"base64",media_type:"application/pdf",data:b64}},{type:"text",text:"You are an academic examiner. Grade this student submission.\n\nAssignment: "+a.title+"\nInstructions: "+(a.description||"Complete the assignment as instructed.")+"\nMax Marks: "+a.max_marks+"\n\nReturn ONLY this JSON: {\"marks\": NUMBER, \"feedback\": \"STRING\"}"}]}]})});const clD=await clRes.json();txt=clD.content?.filter(c=>c.type==="text").map(c=>c.text).join("")||"{}";}
else{const d=await gemRes.json();txt=d.candidates?.[0]?.content?.parts?.[0]?.text||"{}";}const clean=txt.replace(/```json|```/g,"").trim();const parsed=JSON.parse(clean.slice(clean.indexOf("{"),clean.lastIndexOf("}")+1));setGradeMarks(Math.min(a.max_marks,Math.max(0,parsed.marks||0)));setGradeFeedback(parsed.feedback||"");setTimeout(()=>setGrading(sub.id),100);if(addNotif)addNotif("✅","AI Grading Done","Review the suggested marks and feedback, then save.");}catch(e){if(addNotif)addNotif("❌","AI Grading Failed",e.message);}}} style={{background:"none",border:"1px solid "+T.purple,color:T.purple,borderRadius:6,cursor:"pointer",fontSize:11,padding:"5px 12px"}}>🤖 AI Grade</button>}
                                    </div>
                                  </div>
                                  {grading===sub.id&&(
                                    <div style={{marginTop:10,display:"grid",gridTemplateColumns:"120px 1fr auto",gap:8,alignItems:"end"}}>
